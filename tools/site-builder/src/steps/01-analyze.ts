...
import { TemplateAnalyzer, StrapiSchemaGenerator, ImplementationPromptGenerator } from '@octadecimal/template-analyzer';
...
export class AnalyzeStep implements StepHandler {
  ...
  async execute(context: BuildContext): Promise<StepResult> {
    ...
    try {
      ...
      const promptGen = new ImplementationPromptGenerator({
        anthropicApiKey: config.anthropic.apiKey,
        verbose: config.verbose,
      });

      context.implementationPrompt = await promptGen.generate(
        context.analysis,
