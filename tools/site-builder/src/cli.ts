...
program
  ...
  .option('--llm-key <key>', 'LLM provider API key (lub LLM_PROVIDER_API_KEY env)')
  ...
  .action(async (options) => {
    try {
      ...
      const config: SiteBuilderConfig = {
        ...
        anthropic: {
          apiKey: options.llmKey || process.env.LLM_PROVIDER_API_KEY || '',
        },
      ...
      if (!config.anthropic.apiKey) {
        console.error(chalk.red('❌ Brak LLM provider API key. Ustaw LLM_PROVIDER_API_KEY env lub użyj --llm-key'));
        process.exit(1);
      }
