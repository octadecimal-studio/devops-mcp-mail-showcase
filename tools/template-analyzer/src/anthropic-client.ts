...
Wygeneruj JSON schema w formacie Strapi Content Types.`;
  ...
  /**
   * Generuje prompt dla IDE na podstawie analizy i schematu
   */
  async generateImplementationPrompt(analysis: string, schema: string, projectName: string): Promise<string> {
    const systemPrompt = `Jesteś ekspertem od promptowania AI (IDE IDE) dla projektów webowych.

Twoim zadaniem jest wygenerować szczegółowy prompt dla IDE, który pomoże developerowi zaimplementować projekt Next.js + Strapi na podstawie analizy szablonu.
...
Prompt ma być:
- Jasny i szczegółowy
- Gotowy do skopiowania do IDE
- Napisany w języku polskim
- Zawierający konkretne przykłady kodu`;

    const prompt = `Wygeneruj kompletny prompt dla IDE IDE na podstawie:
...
Wygeneruj prompt w formacie Markdown, gotowy do użycia w IDE.`;
