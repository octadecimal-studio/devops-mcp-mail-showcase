...
export class ImplementationPromptGenerator {
  ...
  /**
   * Generuje prompt dla IDE
   */
  async generate(
    analysis: TemplateAnalysis,
    schema: any,
    projectName: string,
    options?: GenerateImplementationPromptOptions
  ): Promise<string> {
    if (this.verbose) {
      console.log('\n=== Implementation Prompt Generator ===');
    ...
    const promptMarkdown = await this.anthropic.generateImplementationPrompt(
      analysisJson,
      schemaJson,
      projectName
    );
   ...
   */
  private async saveToFile(prompt: string, options: GenerateImplementationPromptOptions): Promise<void> {
    const outputDir = options.outputDir || process.cwd();
    const outputFile = options.outputFile || 'IMPLEMENTATION_PROMPT.md';
  ...
  /**
   * Generuje prompt w stylu AGENTS.md (dla workspace rules)
   */
  generateIDERules(analysis: TemplateAnalysis, projectName: string): string {
